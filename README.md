...
| 📧 **域名邮箱** | 多域名邮箱系统：添加域名（自动生成 MX/SPF 解析引导并检测对接）、多账号管理（批量/随机生成、启停、改密、容量）；内置收发信服务，网页收件箱 / 已发送、未读红点、拉框多选批量操作；自研富文本写信（字体/颜色/对齐/列表/图片/附件，图片内嵌 CID） |
...
| 🔄 **网站搬家** | 支持同面板 / 跨面板（含第三方面板）站点一键导出迁移，远程站点自动打包下载导入 |
...
面板内置多域名邮箱系统，无需额外部署 Postfix/Dovecot 等组件，开箱即可收发邮件。

### 开通步骤

1. **添加域名**：进入「邮件 → 域名邮箱」，点「添加域名」，按 3 步向导填写域名、到域名服务商添加解析（A + MX + SPF）、等待自动检测通过。
2. **添加账号**：选中域名 → 「账号管理」→「添加用户」（支持单个 / 批量 / 随机生成，可设容量）。
3. **开始收发**：收件箱、已发送、写信在同一个界面完成。
...
> 面板「添加域名」向导会自动生成以上记录并一键复制，检测 MX 是否指向本机通过后即可收信。

### 收发信端口

| 端口 | 用途 |
|---|---|
| 25 | SMTP 收信（外部邮件投递），同时用于外发到对方邮件服务器 |
| 587 / 465 | 客户端提交发信（SMTP AUTH） |

> 需在云服务器安全组与面板防火墙同时放行；部分云厂商默认封禁 25 端口，外发前请先申请解封。

### 写信

网页端内置富文本编辑器：字体、字号、加粗 / 斜体 / 下划线 / 删除线、字体色与背景色、对齐、有序 / 无序列表、引用、链接、插入图片、附件。

- 图片（≤5MB）以 CID 内嵌方式随邮件发送，收件人无需联网加载外部资源即可查看
- 附件总大小 ≤25MB
- 收件人支持回车逐个添加，粘贴时按 `,` `;` `，` `；` 空格 换行自动拆分

### 说明

- 邮件正文以 maildir 格式存储在数据目录 `mail/<域名>/<账号>/`，索引元数据存于 SQLite
- 收信为实时投递，收件箱自动刷新；左侧菜单「域名邮箱」在有未读邮件时显示红点
- 收件箱支持复选框与鼠标拉框多选，可批量标记已读 / 未读 / 删除
...
| [域名邮箱](#域名邮箱) | 邮箱系统开通步骤、解析记录、收发信端口与写信说明 |
...
│   ├── service/         # 业务逻辑（站点/数据库/Docker/WAF/安全/迁移/邮箱…）
